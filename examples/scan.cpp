...
int main() {
    ...
    }
    {
        ...
        auto get_size = [half_block](size_t n) -> size_t { return round_up((n + 1) / 2, half_block); };
        ...
        // ---- scan actors ----
        auto phase1 = mngr.spawn(prog, kernel_name_1, ndr,
        ...
        auto phase2 = mngr.spawn(prog, kernel_name_2, ndr,
                                 [nd_conf](nd_range &range, message &msg) -> optional<message> {
                                 ...
                                 in_out<uval, mref, mref> {}, in_out<uval, mref, mref> {}, priv<uval, val> {});
        auto phase3 = mngr.spawn(prog, kernel_name_3, ndr,
                                 [nd_conf](nd_range &range, message &msg) -> optional<message> {
                                     return msg.apply([&](uref &data, uref &incs) {
                                         auto size = incs.size();
                                         range = nd_conf(size);
        ...
        self->receive([&](const uvec &results) {
            ...
            for (size_t i = 0; i < problem_size; ++i) {
                cout << setw(6) << i << " | " << setw(6) << values[i] << " | " << setw(6) << results[i] << endl;
            }
        });
    }
