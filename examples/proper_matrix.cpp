...
public:
    ...
    friend typename Inspector::result_type inspect(Inspector &f, square_matrix &m) {
...
void multiplier(event_based_actor *self) {
    ...
    // print "source" matrix
    cout << "calculating square of matrix:" << endl << to_string(m1) << endl;
    ...
    auto box_res = [](fvec &result) -> message { return make_message(matrix_type {move(result)}); };
    ...
    auto worker = mngr.spawn(kernel_source, kernel_name, nd_range {dim_vec {matrix_size, matrix_size}}, unbox_args,
                             box_res, in<float> {}, in<float> {}, out<float> {});

    // send both matrices to the actor and
    // wait for results in form of a matrix_type
    self->request(worker, chrono::seconds(5), move(m1), move(m2)).then([](const matrix_type &result) {
