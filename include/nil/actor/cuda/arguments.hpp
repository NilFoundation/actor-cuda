    ...
    namespace actor {
        ...
        namespace cuda {
            ...
            struct in_out : arg_tag, input_tag, output_tag {
                static_assert(std::is_same<TagIn, val>::value || std::is_same<TagIn, mref>::value,
                              "Argument of type `in_out` must be passed as value or mem_ref.");
                static_assert(std::is_same<TagOut, val>::value || std::is_same<TagOut, mref>::value,
            ...
            struct local : arg_tag, requires_size_tag {
                ...
                template<class F>
                local(size_t size, F fun) : size_(size), fun_ {detail::res_or_none<size_t>(fun)} {
                    // nop
                }
            ...
            struct priv : arg_tag, std::conditional<std::is_same<Tag, val>::value, input_tag, empty_tag>::type {
            ...
            template<class T>
            struct is_val_type : std::integral_constant<bool, !std::is_base_of<is_ref_tag, T>::value> {};
            ...
            struct cl_arg_info_list_impl<detail::type_list<>,
                ...
                using type =
                    typename cl_arg_info_list_impl<detail::type_list<cl_arg_info<in_idx::value, out_idx::value, Arg>>,
                                                   detail::type_list<Remaining...>,
                                                   in_idx::next,
                                                   out_idx::next>::type;
            };

            template<class... Args, class Arg, class... Remaining, int InCounter, int OutCounter>
            ...
            struct cl_arg_info_list_impl<detail::type_list<Args...>, detail::type_list<>, InCounter, OutCounter> {
                using type = detail::type_list<Args...>;
            };

            template<class List>
            struct cl_arg_info_list {
                using type = typename cl_arg_info_list_impl<detail::type_list<>, List, 0, 0>::type;
            };
